...
def calculate_sharpe(
    ...
    This avoids mixing per-trade dispersion with per-day mean, which can inflate Sharpe
    when there are many trades.

    Inputs unchanged:
    - trades must contain close_date and either profit_ratio or profit_abs.
    - starting_balance is used only when profit_ratio is not available.
    """
    if (len(trades) == 0) or (min_date is None) or (max_date is None) or (min_date == max_date):
        return 0.0
    if not starting_balance or starting_balance <= 0:
        # Cannot derive returns from profit_abs safely.
        return 0.0

    # Build a per-trade return series.
    if "profit_ratio" in trades.columns:
        returns = trades.loc[:, ["close_date", "profit_ratio"]].copy()
        value_col = "profit_ratio"
    else:
        returns = trades.loc[:, ["close_date", "profit_abs"]].copy()
        returns["profit_ratio"] = returns["profit_abs"] / starting_balance
        value_col = "profit_ratio"

    # Ensure datetime and drop invalid rows
    returns["close_date"] = pd.to_datetime(returns["close_date"], errors="coerce")
    returns = returns.dropna(subset=["close_date"])

    # Create daily index for the evaluated period
    t_index = pd.date_range(start=min_date, end=max_date, freq="1D", normalize=True)

    # Daily realized returns (sum of trade returns closed that day)
    daily = (
        returns.resample("1D", on="close_date")
        .agg({value_col: "sum"})
        .reindex(t_index)
        .fillna(0.0)
    )

    daily_returns = daily[value_col]

    # Annualize daily Sharpe
    days_in_year = 365.0
    mean = float(daily_returns.mean())
    stdev = float(daily_returns.std())  # pandas default ddof=1

    if stdev != 0.0 and not np.isnan(stdev):
        sharpe = mean / stdev * math.sqrt(days_in_year)
    else:
        # Keep existing "clearly not optimal" behavior consistent with other metrics.
        sharpe = -100.0
