...
class ParquetDataHandler(IDataHandler):
    ...
    def _ohlcv_load(
        ...
        try:
            ...
            optional_columns = ['volume_total', 'contract', 'bid','ask','volume_none','trades','trades_bid','trades_ask','trades_none','orderflow']

            # Выбираем те optional, которые реально есть в parquet
            present_optional_columns = [col for col in optional_columns if col in pairdata.columns]

            # Итоговые колонки, которые оставим в датафрейме
            keep_columns = [col for col in base_columns if col in pairdata.columns] + present_optional_columns
            pairdata = pairdata[keep_columns]

            # Приведение типов только для колонок, которые есть
            type_mapping = {
                "open": "float",
                "high": "float",
                "low": "float",
            ...
            for col, dtype in type_mapping.items():
                if col in pairdata.columns:
                    pairdata[col] = pairdata[col].astype(dtype)
