...
class ExchangeResolver(IResolver):
    ...
    def load_exchange(
        ...
        *,
        ...
        if exchange_name_lower in ['usa_stocks','usa_futures','forex']:
            try:
                logger.info(f"Loading custom exchange: {exchange_name}")
                if exchange_name_lower == 'usa_stocks':
                    from freqtrade.exchange.usa_stocks import USA_Stocks
                    return USA_Stocks(config, validate=validate)
                if exchange_name_lower == 'usa_futures':
                    from freqtrade.exchange.usa_futures import USA_Futures
                    return USA_Futures(config, validate=validate)
                if exchange_name_lower == 'forex':
                    from freqtrade.exchange.forex import Forex
                    return Forex(config, validate=validate)
            except ImportError as e:
                raise OperationalException(
                    f"Custom exchange {exchange_name} not found. "
                    f"Error: {e}"
                )
